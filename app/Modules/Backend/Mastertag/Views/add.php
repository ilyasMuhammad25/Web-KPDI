...
<div class="app-main__inner">
    <div class="app-page-title">
        <div class="page-title-wrapper">
            <div class="page-title-heading">
                ...
                <div><?=lang('Mastertag.action.add')?> <?=lang('Mastertag.module')?> <div class="page-title-subheading"><?=lang('Mastertag.form.complete_the_data')?>.</div>
                </div>
            </div>
    ...
    <div class="main-card mb-3 card">
        ...
        <div class="card-body">
            <div id="infoMessage"><?=$message ?? ''?></div> <?=get_message('message')?> <form id="frm_create"
                class="col-md-12 mx-auto" method="post" action="<?=base_url('Mastertag/create')?>" onsubmit="return validateForm()">
                <div class="form-row">
                    <div class="col-md-6">
                ...
                <div class="form-group">
                    ...
                    <div>
                        <input type="number" class="form-control" id="frm_create_length" min="1" max="255" name="length"
                            placeholder="<?=lang('Mastertag.field.length')?> " value="<?=set_value('length', 0)?>" />
                    </div>
                </div>
                ...
                <div id="dynamic_field">
                    <div id="row0">
                        ...
                        <div class="form-group col-md-2" style="display: inline-block">
                            ...
                            <div>
                                <input type="text" class="form-control" id="frm_create_fielddatas_name"
                                    name="fielddatas_name[0]" placeholder="<?=lang('Mastertag.fielddatas.name')?> " value="<?=set_value('fielddatas_name[0]')?>" />
                            </div>
                        </div>
                        <div class="form-group col-md-2" style="display: inline-block">
                            <label for="delimiter"><?=lang('Mastertag.fielddatas.delimiter')?> </label>
                            <div>
                                <input type="text" class="form-control" id="frm_create_delimiter" name="delimiter[0]"
                                    placeholder="<?=lang('Mastertag.fielddatas.delimiter')?> " value="<?=set_value('delimiter[0]')?>" />
                            </div>
                        </div>
                        <div class="form-group col-md-2" style="display: inline-block">
                            <label for="sortno"><?=lang('Mastertag.fielddatas.sortno')?> </label>
                            <div>
                                <input type="text" class="form-control" id="frm_create_sortno" name="sortno[0]"
                                    placeholder="<?=lang('Mastertag.fielddatas.sortno')?> " value="<?=set_value('sortno[0]')?>" />
                            </div>
                        </div>
                ...
                <div id="dynamic_field1s">
                    <div id="row1s0">
                        <div class="form-group col-md-2" style="display: inline-block">
                            ...
                            <div>
                                <input type="text" class="form-control" id="frm_create_fieldindicator1s_code"
                                    name="code1s[0]" placeholder="<?=lang('Mastertag.fieldindicator1s.code')?> " value="<?=set_value('code1s[0]')?>" />
                            </div>
                        </div>
                        <div class="form-group col-md-2" style="display: inline-block">
                            <label for="name"><?=lang('Mastertag.fieldindicator1s.name')?> </label>
                            <div>
                                <input type="text" class="form-control" id="frm_create_fieldindicator1s_name"
                                    name="name1s[0]" placeholder="<?=lang('Mastertag.fieldindicator1s.name')?> " value="<?=set_value('name1s[0]')?>" />
                            </div>
                ...
                <div id="dynamic_field2s">
                    <div id="row2s0">
                        <div class="form-group col-md-2" style="display: inline-block">
                            ...
                            <div>
                                <input type="text" class="form-control" id="frm_create_fieldindicator2s_code"
                                    name="code2s[0]" placeholder="<?=lang('Mastertag.fieldindicator2s.code')?> " value="<?=set_value('code2s[0]')?>" />
                            </div>
                        </div>
                        <div class="form-group col-md-2" style="display: inline-block">
                            <label for="name"><?=lang('Mastertag.fieldindicator2s.name')?> </label>
                            <div>
                                <input type="text" class="form-control" id="frm_create_fieldindicator2s_name"
                                    name="name2s[0]" placeholder="<?=lang('Mastertag.fieldindicator2s.name')?> " value="<?=set_value('name2s[0]')?>" />
                            </div>
...
$(document).ready(function() {
    ...
    $('#add').on("click", function() {
        $('#dynamic_field').append('<div  id="row' + i + '">' +
            ...
            '<input type="text" class="form-control" id="frm_create_fielddatas_code[' + i +
            ']" name="code[' + i + ']" placeholder="' + code + '" value="" />' + '</div>' +
            '</div>&nbsp;' + '<div class="form-group col-md-2" style="display: inline-block">' +
            '<label for="name">' + name + '</label>' + '<div>' +
            '<input type="text" class="form-control" id="frm_create_fielddatas_name[' + i +
            ']" name="fielddatas_name[' + i + ']" placeholder="' + name + '" value="" />' +
            '</div>' + '</div>&nbsp;' +
            '<div class="form-group col-md-2" style="display: inline-block">' +
            '<label for="delimiter">' + delimiter + '</label>' + '<div>' +
            '<input type="text" class="form-control" id="frm_create_fielddatas_delimiter[' + i +
            ']" name="delimiter[' + i + ']" placeholder="' + delimiter + '" value="" />' +
            '</div>' + '</div>&nbsp;' +
            '<div class="form-group col-md-2" style="display: inline-block">' +
            '<label for="sortno">' + sortno + '</label>' + '<div>' +
            '<input type="text" class="form-control" id="frm_create_fielddatas_sortno[' + i +
            ']" name="sortno[' + i + ']" placeholder="' + sortno + '" value="" />' + '</div>' +
            '</div>&nbsp;' + '<div class="form-group col-md-1" style="display: inline-block">' +
            '<label for="isshow">' + isshow + '</label>' + '<div>' +
            '<input type="checkbox" name="isshow[' + i + ']" value="1">' + '</div>' +
            '</div>&nbsp;' + '<div class="form-group col-md-1" style="display: inline-block">' +
            '<label for="repeatable">' + repeatable + '</label>' + '<div>' +
    ...
    $('#add1s').on("click", function() {
        $('#dynamic_field1s').append('<div  id="row1s' + i_1s + '">' +
            '<div class="form-group col-md-2" style="display: inline-block">' +
            '<label for="code1s">' + code1s + '</label>' + '<div>' +
            '<input type="text" class="form-control" id="frm_create_fieldindicator1s_code[' + i_1s +
            ']" name="code1s[' + i_1s + ']" placeholder="' + code1s + '" value="" />' + '</div>' +
            '</div>&nbsp;' + '<div class="form-group col-md-2" style="display: inline-block">' +
            '<label for="name1s">' + name1s + '</label>' + '<div>' +
            '<input type="text" class="form-control" id="frm_create_fieldindicator1s_name[' + i_1s +
            ']" name="name1s[' + i_1s + ']" placeholder="' + name1s + '" value="" />' +
            '</div></div><button id="' + i_1s +
            '_1s" class="btn_remove1s btn btn-danger">x</button>' + '</div>');
        i_1s++;
    });
    ...
    $('#add2s').on("click", function() {
        $('#dynamic_field2s').append('<div  id="row2s' + i_2s + '">' +
            '<div class="form-group col-md-2" style="display: inline-block">' +
            '<label for="code2s">' + code2s + '</label>' + '<div>' +
            '<input type="text" class="form-control" id="frm_create_fieldindicator2s_code[' + i_2s +
            ']" name="code2s[' + i_2s + ']" placeholder="' + code2s + '" value="" />' + '</div>' +
            '</div>&nbsp;' + '<div class="form-group col-md-2" style="display: inline-block">' +
            '<label for="name2s">' + name2s + '</label>' + '<div>' +
            '<input type="text" class="form-control" id="frm_create_fieldindicator2s_name[' + i_2s +
            ']" name="name2s[' + i_2s + ']" placeholder="' + name2s + '" value="" />' +
            '</div></div><button id="' + i_2s +
            '_2s" class="btn_remove2s btn btn-danger">x</button>' + '</div>');
        i_2s++;
    });
