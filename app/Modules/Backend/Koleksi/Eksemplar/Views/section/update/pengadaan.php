				...
				<div data-parent="#accordion3" id="collapse3" class="collapse show" style="">
					<div class="card-body">
							...
							<div class="card-body">
								<table style="width: 100%;" id="eksemplar-tbl"
									...
									<tbody id="eksemplar-tbody">
										<tr>
											<td><input name="barcode_no" type="text" class="form-control barcode" value="<?=set_value('barcode_no', $eksemplar->barcode_no)?>"  readonly></td>
											<td><input name="register_no" type="text" class="form-control barcode" value="<?=set_value('register_no', $eksemplar->register_no)?>"  readonly></td>
											<td><input name="rfid" type="text" class="form-control barcode" value="<?=set_value('rfid', $eksemplar->rfid)?>" readonly></td>
											<td><input name="call_no" type="text" class="form-control barcode" value="<?=set_value('call_no', $eksemplar->call_no)?>" ></td>
										</tr>
									</tbody>
								</table>
							</div>
									...
									<div>
										<select class="form-control" name="source_type_id" tabindex="-1" aria-hidden="true" placeholder="<?=lang('Eksemplar.field.Jenissumber')?>">
											...
											<?php foreach (get_ref('ref_source') as $row): ?>
												<option value="<?=$row->id?>" <?=($eksemplar->source_type_id == $row->id)?'selected':''?>><?=$row->name?></option>
											<?php endforeach;?>
										</select>
									</div>
								</div>
									...
									<div>
										<select class="form-control" name="source_name_id" tabindex="-1" aria-hidden="true" placeholder="<?=lang('Eksemplar.field.Namasumber')?>">
											...
											<?php foreach (get_ref('ref_partner') as $row): ?>
												<option value="<?=$row->id?>" <?=($eksemplar->source_name_id == $row->id)?'selected':''?>><?=$row->name?></option>
											<?php endforeach;?>
										</select>
									</div>
								</div>
									...
									<div class="select-wrapper">
										<select class="form-control select2" name="media_type_id" style="width:100%">
											...
											<?php foreach(get_dropdown('m_media_eksemplar') as $row):?>
												<option value="<?=$row->code?>" <?=($eksemplar->media_type_id == $row->code)?'selected':''?>><?=$row->text?></option>
											<?php endforeach;?>
										</select>
									</div>
								</div>
									...
									<div>
										<select class="form-control" name="access_id" placeholder="<?=lang('Eksemplar.field.Akses')?>">
											...
											<?php foreach (get_ref('ref_akses') as $row): ?>
												<option value="<?=$row->id?>" <?=($eksemplar->access_id == $row->id)?'selected':''?>><?=$row->name?></option>
											<?php endforeach;?>
										</select>
									</div>
								</div>
									...
									<div>
										<select class="form-control" name="location_library_id" id="location_library_id" style="width:100%">
											...
											<?php foreach(get_dropdown('m_lokasiperpustakaan') as $row):?>
												<option value="<?=$row->code?>" <?=($eksemplar->location_library_id == $row->code)?'selected':''?>><?=$row->text?></option>
											<?php endforeach;?>
										</select>
									</div>
								</div>
								...
								<div class="position-relative form-group">
									...
									<div>
										<select class="form-control" name="location_room_id" id="location_room_id" style="width:100%" data-url="<?=base_url('api/eksemplar/location')?>">
											<option value="">-Pilih-</option>
											<?php foreach(get_dropdown('m_lokasiruang','Lokasi_perpustakaan_id = '.$eksemplar->location_library_id) as $row):?>
												<option value="<?=$row->code?>" <?=($eksemplar->location_room_id == $row->code)?'selected':''?>><?=$row->text?></option>
											<?php endforeach;?>
										</select>
									</div>
								</div>
						...
						<div class="form-row">
									...
									<div class="select-wrapper">
										<select class="form-control select2" name="currency" placeholder="<?=lang('Eksemplar.field.currency')?>" style="width:100%">
											...
											<?php foreach(get_dropdown('m_currency',null,'Description','Description') as $row):?>
												<option value="<?=$row->code?>" <?=($eksemplar->currency == $row->code)?'selected':''?>><?=$row->code?></option>
											<?php endforeach;?>
										</select>
									</div>
								</div>
							...
							<div class="col-md-4">
								<div class="position-relative form-group">
									...
									<div>
										<input type="number" class="form-control" name="price" value="<?=set_value('price', $eksemplar->price)?>" placeholder="<?=lang('Eksemplar.field.Price')?> " />
									</div>
								</div>
							</div>
							<div class="col-md-4">
								<div class="position-relative form-group">
									...
									<div>
										<select class="search form-control" id="price_type" name="price_type">
											<option value="Per jilid" <?=($eksemplar->price_type == 'Per jilid')?'selected':''?>>Per Jilid</option>
											<option value="Per eksemplar" <?=($eksemplar->price_type == 'Per eksemplar')?'selected':''?>>Per Eksemplar</option>
										</select>
									</div>
								</div>
							</div>
						...
						<div class="form-row">
							<div class="col-md-4">
								<div class="position-relative form-group">
									...
									<div>
										<input type="date" class="form-control" name="procurement_date" value="<?=set_value('procurement_date', $eksemplar->procurement_date)?>" placeholder="Tanggal Pengadaan" />
									</div>
								</div>
							</div>
							<div class="col-md-4">
									...
									<div class="select-wrapper">
										<select class="form-control select2" name="availability_id" placeholder="<?=lang('Eksemplar.field.Ketersediaan')?>" style="width:100%">
											...
											<?php foreach (get_ref('ref_status') as $row): ?>
												<option value="<?=$row->id?>" <?=($eksemplar->availability_id == $row->id)?'selected':''?>><?=$row->name?></option>
											<?php endforeach;?>
										</select>
									</div>
								</div>
						...
						<div>
							...
							<input type="checkbox" class="iCheck-square" name="is_opac" id="is_opac" value="1" <?=($eksemplar->is_opac == 1)?'checked':''?>>
