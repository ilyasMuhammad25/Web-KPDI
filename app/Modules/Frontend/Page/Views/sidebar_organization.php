...
$organizationModel = new \App\Modules\Backend\Organization\Models\OrganizationModel();
$categories = $organizationModel
	->select('c_references.*')
	->select('count(t_organization.category_id) as total')
	->join('c_references','c_references.id = t_organization.category_id','inner')
	->groupBy('c_references.id')
	->findAll();
?>
...
<aside class="col-12 col-xl-3 offset-xl-1 col-lg-4 col-md-7 blog-sidebar lg-padding-4-rem-left md-padding-15px-left">
    ...
    <div class="margin-5-rem-bottom xs-margin-35px-bottom wow animate__fadeIn">
        <span class="alt-font font-weight-500 text-large text-extra-dark-gray d-block margin-35px-bottom">Kategori</span>
        <ul class="list-style-07 list-unstyled">
            <?php foreach($categories as $row):?>
            <li><a href="<?=base_url('organisasi?slug='.$row->slug)?>"><?=$row->name?></a><span class="item-qty"><?=$row->total ?? 0?></span></li>
            <?php endforeach;?>
        </ul>
    </div>
    <!-- <div class="margin-5-rem-bottom xs-margin-35px-bottom wow animate__fadeIn">
